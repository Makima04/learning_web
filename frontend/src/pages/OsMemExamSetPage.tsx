// 408 OS 内存管理题集：大类入口 → 按卷序刷 / 快速校对；点开题目才出小类标签。
...
type GroupParam = OsMemGroupId | "all";

function isGroupParam(raw: string | undefined): raw is GroupParam {
  return raw === "all" || OS_MEM_GROUPS.some((g) => g.id === raw);
}
...
export function OsMemExamSetPage() {
    ...
    return (
      <div className="mx-auto max-w-3xl p-4 md:p-6">
        ...
        <Button asChild variant="link" className="px-0">
          <Link to={osMemSetPath()}>返回题集</Link>
        </Button>
      </div>
    );
  }
...
function Hub() {
  ...
  const allDone = all.filter((e) => marked.has(osMemExamKey(e.year, e.n))).length;
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 p-4 md:p-6">
      <div>
        ...
        <h1 className="mt-1 text-xl font-semibold">OS 内存管理题集</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          2012–2026 共 {all.length} 道。先选大类，或按 408 卷序一把校对；点进题目才出小类标签。
        </p>
      </div>
      ...
      <Card className="border-primary/30">
        <CardContent className="flex flex-wrap items-center justify-between gap-3 p-4">
          ...
          <Button asChild size="sm">
            <Link to={osMemSetPath({ group: "all", mode: "proof" })}>快速校对</Link>
          </Button>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {OS_MEM_GROUPS.map((g) => {
          const exams = osMemExamsInOrder(g.id);
          const done = exams.filter((e) => marked.has(osMemExamKey(e.year, e.n))).length;
          return (
            <Card key={g.id}>
              ...
              <CardContent className="flex flex-wrap gap-2 pb-4">
                <Button asChild size="sm">
                  <Link to={osMemSetPath({ group: g.id, mode: "proof" })}>快速校对</Link>
                </Button>
                <Button asChild size="sm" variant="outline">
                  <Link to={osMemSetPath({ group: g.id })}>进入题集</Link>
                </Button>
              </CardContent>
            </Card>
          );
...
function SetView({ group }: { group: GroupParam }) {
  ...
  const topicParam = params.get("topic") as OsMemTopicId | null;
  const qKey = params.get("q");
  const g = group === "all" ? undefined : osMemGroup(group);
  const topicFilter =
    topicParam && osMemTopic(topicParam) && (!g || osMemTopic(topicParam)?.kpId === g.kpId)
      ? topicParam
      : undefined;

  const refs = useMemo(
    () => osMemExamsInOrder(group, topicFilter),
    [group, topicFilter]
  );
  ...
  useEffect(() => {
    ...
    setPapers(null);
    setErr("");
    loadCs408ExamPapers(osMemYears())
      .then((list) => {
        if (cancelled) return;
  ...
  const openKey = qKey && refs.some((r) => osMemExamKey(r.year, r.n) === qKey) ? qKey : null;
  ...
  const onMark = useCallback(
    (row: Row, mark: MarkLevel) => {
      const pk = (row.item && primaryKpId(row.item)) || osMemTopic(row.ref.topic)?.kpId;
      if (!pk) return;
      markItem({
      ...
      if (after) setOpen(osMemExamKey(after.ref.year, after.ref.n));
    },
    [markItem, proof, visible, markMap, setOpen]
  );

  const title = group === "all" ? "全部 · 408 卷序" : (g?.name ?? group);
  const topics = group === "all" ? [] : osMemTopicsForGroup(group);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 p-4 md:p-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {group !== "all" && (
          <Link to={`/viz/${g?.kpId ?? ""}`} className="rounded-md px-3 py-1 text-xs text-muted-foreground hover:underline">
            图解
          </Link>
        )}
      </div>
      ...
      {topics.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          <Link
            to={osMemSetPath({ group, mode: proof ? "proof" : undefined })}
            className={cn(
          ...
          {topics.map((t) => (
            <Link
              key={t.id}
              to={osMemSetPath({ group, topic: t.id, mode: proof ? "proof" : undefined })}
              className={cn(
                "rounded-md px-2 py-0.5 text-[11px]",
                topicFilter === t.id ? "bg-violet-600 text-white" : "bg-violet-500/10 text-violet-800 dark:text-violet-200"
              )}
            >
              {t.name}
            </Link>
      ...
      <div className="space-y-2">
        {visible.map((row) => {
          const key = osMemExamKey(row.ref.year, row.ref.n);
...
function QRow({
  ...
  return (
    <Card id={`q-${osMemExamKey(ref.year, ref.n)}`} className={cn(open && "ring-1 ring-primary/40")}>
      <button type="button" className="w-full px-4 py-3 text-left" onClick={onToggle}>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <OsMemTags year={ref.year} n={ref.n} showMinor={open} compact link={false} />
          {mark && (
            <span
              className={cn(
      ...
      {open && (
        <CardContent className="space-y-3 border-t pt-3">
          {topic && (
            <p className="text-xs text-muted-foreground">
              小类「{topic.name}」· {ref.hook}
              {group && viz ? (
                <>
                  {" · "}
                  <Link to={`/viz/${group.kpId}`} className="text-primary hover:underline">
                    图解 {topic.vizHint}
                  </Link>
                </>
              ) : null}
